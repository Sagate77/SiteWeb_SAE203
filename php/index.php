<?php
session_start();
if (!isset($_SESSION['email']) || $_SESSION['role'] !== 'Etudiant') {
    header("Location: connexion.php");
    exit();
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Reservation de Materiels</title>
  <link rel="stylesheet" href="../css/index.css">
</head>
<body>
  <nav>
    <ul class="nav-menu">
        <li class="left"><a href=""><img src="../imagesSite/test.png" alt="logo"></a></li>
        <li><a href="html/norvege.html">Materiels</a></li>
        <li><a href="html/opera.html">Reservation</a></li>
        <li class="right"><a href="html/propos.html">Mon Compte</a></li>
    </ul>
</nav>
  <div class="Texte">
    <h1>Reservation de materiels</h1>
  </div>
  <img src="../imagesSite/iut2.jpg" alt="Avion en vol" id="ImageTaille">
  <section class="destination-description">
    <div class="text">
      <p>
      <strong>Bienvenue sur la plateforme de réservation de matériels de l'IUT Gustave Eiffel !</strong>
      </p>
      <p>
        Ce site a été spécialement conçu pour permettre aux étudiants, enseignants et personnels de l’IUT de gérer facilement l’emprunt de matériel pédagogique, technique ou multimédia.
        Grâce à une interface simple et intuitive, vous pouvez consulter la disponibilité des équipements, effectuer vos réservations en quelques clics, et suivre l’historique de vos emprunts.
      </p>
      <p>
        Que ce soit pour un projet, un cours ou un événement, notre plateforme facilite l’accès aux ressources matérielles dont vous avez besoin au quotidien.
      </p>
    </div>
    <div class="image">
      <img src="../images SAE 201-203/Salle138.JPG" alt="Panneau de destinations">
    </div>
  </section>
  <main>


  <footer>
    <div class="footer-container">
      <div class="footer-middle">
        <p>2024-2025 BUT MMI</p>
        <img src="../imagesSite/test.png" alt="Logo MMI" class="footer-logo">
      </div>
      <div class="footer-links">
        <a href="html/propos.html">À propos</a>
        <a href="html/propos.html">Mentions légales</a>
      </div>
    </div>
  </footer>
</body>
</html>
